...
const AddExpense = () => {
    ...
    const onSubmit: SubmitHandler<FormValues> = async (data: FormValues) => {
        setError('')
        try {
            const formatDate = normalizeDate(data.date)
            ...
            toast.error('Error al añadir el nuevo gasto!')
        }
    }
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col w-full bg-background-dark py-2 px-4 gap-2 mt-4 rounded-lg border border-secondary text-text-secondary">
                        <div className="mb-4">
                            ...
                            <input
                                ...
                                {...register('title', {
                                    required: 'Debe ingresar el titulo del gasto'
                                })}
                            ...
                            />
                            {errors.title && <p className='text-status-error text-sm'>{(errors.title as { message?: string }).message}</p>}
                        </div>
                        <div className="mb-4">
                            ...
                            <input
                                ...
                                placeholder="50.000"
                                {...register('amount', {
                                    required: 'Debe ingresar el monto del gasto'
                                })}
                                className="w-full p-3 border border-border rounded-lg bg-background-neutral focus:outline-none focus:ring-2 focus:ring-primary"
                            />
                            {errors.amount && <p className='text-status-error text-sm'>{(errors.amount as { message?: string }).message}</p>}
                        </div>
                        <div className="mb-4">
                            ...
                            <input
                                ...
                                {...register('date', {
                                    required: 'Debe ingresar la fecha del gasto',
                                    pattern: {
                        ...
                        <div className="mb-4">
                            ...
                            <input
                                ...
                                {...register('category', {
                                    required: 'Debe ingresar la categoria del producto'
                                })}
                                className="w-full p-3 border border-border rounded-lg bg-background-neutral focus:outline-none focus:ring-2 focus:ring-primary"
                            />
                            {errors.category && <p className='text-status-error text-sm'>{(errors.category as { message?: string }).message}</p>}
                        </div>
